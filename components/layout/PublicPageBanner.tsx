...
export function PublicPageBanner({ src, title, subtitle }: PublicPageBannerProps) {
  ...
  if (!imageSrc) {
    return (
      <section className="border-b border-line bg-graphite-light/40 px-4 py-5 md:px-6 md:py-6">
        <div className="max-w-6xl mx-auto w-full">
          <h1 className="font-display text-2xl md:text-3xl tracking-wide text-foreground">{title}</h1>
          {subtitle ? (
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex h-full flex-col justify-end px-4 pb-5 md:px-6 md:pb-6 max-w-6xl mx-auto w-full">
        <h1 className="font-display text-2xl md:text-3xl tracking-wide text-white">{title}</h1>
        {subtitle ? (
